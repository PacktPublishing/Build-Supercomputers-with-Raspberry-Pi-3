...
/* Update values for each point along string */
  for (i = 1; i <= nsteps; i++)
  {
    ...
    if(first != 1) 
     ...
     }// End of for(j = 1; j <= npoints; j++)
     for(j = 1; j <= npoints; j++)
     {
       oldval[j] = values[j];
       values[j] = newval[j];
     }
  }// End of for (i = 1; i <= nsteps; i++)
}// End of void update(int left, int right)
...
//  draw_wave(&results[0]); //                     <===================>
}// End of void output_master(void)
...
}// End of void output_workers(void)
